...
class Embedding_based(nn.Module):
    ...
    def __init__(self, args, n_users, n_items, n_entities, n_relations):
        ...
        self.inject_embedding_type = args.inject_embedding_type
    ...
    def inject_concat(self, item_embed, item_kg_embed):
        ...
        fc = nn.Linear(2 * self.embed_dim, self.embed_dim)
        relu = nn.ReLU()
        return relu(fc(concat_embed))
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids):
        ...
        item_pos_embed = self.item_embed(
            item_pos_ids)  # (cf_batch_size, embed_dim)
        item_neg_embed = self.item_embed(
            item_neg_ids)  # (cf_batch_size, embed_dim)
        ...
        item_pos_cf_embed = inject_embedding(
            item_pos_embed, item_pos_kg_embed)  # (cf_batch_size, embed_dim)
        item_neg_cf_embed = inject_embedding(
            item_neg_embed, item_neg_kg_embed)  # (cf_batch_size, embed_dim)

        pos_score = torch.sum(user_embed * item_pos_cf_embed,
                              dim=1)  # (cf_batch_size)
        neg_score = torch.sum(user_embed * item_neg_cf_embed,
                              dim=1)  # (cf_batch_size)

        cf_loss = (-1.0) * torch.log(1e-10 + F.sigmoid(pos_score - neg_score))
        cf_loss = torch.mean(cf_loss)

        l2_loss = _L2_loss_mean(user_embed) + _L2_loss_mean(
            item_pos_cf_embed) + _L2_loss_mean(item_neg_cf_embed)
    ...
    def calc_score(self, user_ids, item_ids):
        ...
        item_cf_embed = inject_embedding(item_embed,
                                         item_kg_embed)  # (n_items, embed_dim)

        cf_score = torch.matmul(user_embed, item_cf_embed.transpose(
            0, 1))  # (n_users, n_items)
